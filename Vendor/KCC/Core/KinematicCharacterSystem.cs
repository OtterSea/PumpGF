...
namespace KinematicCharacterController
{
    ...
    public class KinematicCharacterSystem : MonoBehaviour
    {
        ...
        //PUMP改：添加一个供外界调用的手动刷新函数 在AutoSimulation为false时外界调用
        public static void ExecuteSimulationUpdate(float deltaTime)
        {
            if(deltaTime <= 0f)
            {
                throw new Exception("KCCSystem deltaTime illegal");
            }
            if (Settings.Interpolate)
            {
                PreSimulationInterpolationUpdate(deltaTime);
