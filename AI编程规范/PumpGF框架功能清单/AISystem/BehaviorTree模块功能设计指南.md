...
| 14. 命名空间 | `PumpGF.AI.BehaviorTree` | 见 §13 |
| 9. GOAP / HTN | ❌ **明确不做** | 目标规划型 AI，动作游戏 Boss 用不上，属于过度设计 |
| 10. BT 与 FSM 关系 | 二者并存、互不依赖，可组合（FSM 状态内挂 BT / BT 叶节点驱动 FSM） | 见「§11 BT 与 FSM 的分工与协作」 |
| 11. 事件驱动重评估 | 支持基于 Blackboard 变化的条件中止（`Abort` / Reactive Decorator） | 应对黑魂式 Boss「被打断→切换招式」的反应式需求 |
...
BTNode（抽象基类）
  ...
  │   └─ ReactiveNode      // 监听黑板 Key，变化时中止子树重评估（事件驱动）
  └─ LeafNode（无子节点，真正干活）
...
public sealed class BehaviorTreeBuilder
{
    ...
    public BehaviorTreeBuilder Reactive(string watchKey); // 监听黑板 Key 变化中止重评估
...
var tree = BehaviorTreeBuilder.Create("Boss")
    .Selector()                                   // 从上到下择一
        // —— 阶段：血量低于 30% 进入狂暴（Reactive 监听血量，随时打断当前招式）
        .Reactive(BBKeys.HpRatio)
        .Sequence()
...
> 说明：`Reactive(BBKeys.HpRatio)` 装饰器监听黑板血量比，一旦跨越阈值即中止当前 Running 子树、触发重评估，从而实现「被打到残血→立刻切狂暴」的反应式打断——这正是行为树相对 FSM 在 Boss 战上的核心优势。
...
| 4 | 命名空间 | ✅ `PumpGF.AI.BehaviorTree`（见 §13、§0 决策 14） |
